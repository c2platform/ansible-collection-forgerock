...
|-------------------------|--------|-------|-------|------------------------------------------------------------------------------------------------------------------------------|
|am_amster_scripts_default|no      |[defaults](./defaults/main.yml)|       |List of Amster scripts that are always created. These type of scripts will not run multiple times but not or once. This is different from scripts defined in `am_amster_templates`.|
|am_amster_subscripts     |no      |[defaults](./defaults/main.yml)|       |List of Amster scripts that are called by other Amster scripts. These will always be created regardless of what you configure.|
|am_amster_templates      |no      |[defaults](./defaults/main.yml)|       |List of templates for creating Amster scripts. It also defines the scripts with required and optional variables.              |
